...
allTools :: [Tools.Tool]
allTools = [Tools.ToolOpenFile, Tools.ToolFocusFile, Tools.ToolCloseFile, Tools.ToolAppendFile, Tools.ToolInsertInFile, Tools.ToolEditFileByMatch, Tools.ToolDeleteLineByMatch, Tools.ToolReplaceInFile, Tools.ToolAddDependency, Tools.ToolSummariseAction, Tools.ToolPanic, Tools.ToolReturn]
...
getTask :: AppState -> [Tools.Tool] -> IsNestedAiFunc -> IsCloseFileTask -> Text -> Text
getTask st tools isNestedAiFunc isCloseFileTask mainTask = do
  let res = stateCompileTestRes st
      hasEditTool = any Tools.isEditor tools
...
contextToMessages Context{..} tools theState isNestedAiFunc isCloseFileTask exampleReturn = do
  ...
  where
    toolDesc = Tools.toolsToDescription tools
    embeddingBackground = "You are running embedded in a NOndeterministic higher-order programming language (NOTPL), and hence have the ability to 'Return' values in a certain format, which are converted to datatypes in the host language. You also have access to various tools you can use, depending on the task. The tools available and the Return format are described in detail further down below. If you don't Return, after applying a tool you'll be called/prompted again with the new state post-tool-application, so you don't need to finish each task in a single response. Some tasks will ask you to just directly calculate and Return some value, while some will ask you to e.g. modify files on disk then Return after that's done. You may also see an overall plan, before seeing your current task; after finishing you current task you should Return straight away, not continue to carry out other tasks in the plan (the overall plan is just for your reference). The higher-order program you're embedded in will handle prompting you to complete the remaining tasks after Returning. Any text you output that's not part of a tool call or Return will be ignored, however there's a SummariseAction tool (described in more detail further below) that you can use to describe your intention and future plans."
